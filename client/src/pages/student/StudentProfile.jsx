...
export default function StudentProfile() {
  ...
  return (
    <div className="ip-app-wrapper" style={{ display: "flex", height: "100vh", background: "var(--bg)" }}>
      <Sidebar role="student" />
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <StudentTopbar title="My Profile" />
        
        <main style={{ flex: 1, overflowY: "auto" }}>
          <PageHeader title="Profile Settings" subtitle="View your CareerSync profile data" />
            ...
            <div style={{ maxWidth: 700, margin: "0 auto", marginTop: "var(--space-6)" }}>
              {loading ? (
                ...
                <Card style={{ overflow: "hidden" }}>
                  <div style={{ height: 120, background: "linear-gradient(135deg, var(--accent) 0%, #178361 100%)" }}></div>
                  <div style={{ padding: "0 var(--space-6) var(--space-6)", position: "relative" }}>
                    <div style={{ width: 100, height: 100, borderRadius: "50%", background: "var(--color-bg-panel)", border: "4px solid var(--color-bg-panel)", marginTop: -50, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40, fontWeight: 600, color: "var(--accent)", boxShadow: "var(--shadow-sm)", marginBottom: "var(--space-4)" }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap: "var(--space-4)" }}>
                      ...
                      {profile.resumeLink && (
                        <div style={{ padding: "var(--space-4)", background: "var(--color-bg-panel-sunken)", borderRadius: "var(--radius-sm)", border: "1px solid var(--color-border-subtle)" }}>
                          <p style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)", margin: "0 0 var(--space-2)", fontWeight: 600 }}>Resume</p>
                          <a href={profile.resumeLink} target="_blank" rel="noreferrer" style={{ fontSize: 15, color: "var(--accent)", textDecoration: "none", fontWeight: 500 }}>View Resume ↗</a>
                        </div>
                      )}
                    </div>
                  </div>
